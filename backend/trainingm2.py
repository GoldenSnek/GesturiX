...
if torch.cuda.is_available():
    ...
    print("   3. PyTorch with CUDA: pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu118")

num_classes = 29  # Kaggle dataset has 29 classes (A-Z + del + nothing + space)
batch_size = 64
...
# Since test folder only has 28 sample images, we'll split the train data into train/val/test
local_train_path = "D:/DatasetSign/archive/asl_alphabet_train/asl_alphabet_train"

# Load all the training data
print("Loading training data...")
hf_dataset_all = load_dataset("imagefolder", data_dir=local_train_path, split="train")

# Split into 70% train, 15% val, 15% test
dataset_size = len(hf_dataset_all)
...
try:
    class_labels_map = hf_dataset_all.features["label"].names
    if len(class_labels_map) != num_classes:
        print(f"🚨 WARNING: Expected {num_classes} classes, but detected {len(class_labels_map)} classes.")
        print(f"Detected class names: {class_labels_map}")
        # Update num_classes to match detected classes
        num_classes = len(class_labels_map)
except:
    print("⚠️ Could not automatically detect class names. Using default labels.")
    class_labels_map = [chr(ord("A")+i) for i in range(26)] + ["del", "nothing", "space"]

print(f"✅ Loaded dataset with {len(train_dataset)} train, {len(val_dataset)} val, {len(test_dataset)} test samples")
print(f"✅ Number of classes: {num_classes}")
print(f"✅ Class labels: {class_labels_map}")
...
disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=class_labels_map)
...
sorted_labels = [class_labels_map[i] for i in sorted_idx]
...
print(f"✅ Per-class accuracy graph saved as per_class_accuracy_{model_name}.png")
