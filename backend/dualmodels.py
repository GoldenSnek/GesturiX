...
class DualModelPredictor:
    ...
    def predict(self, features):
        """
        Predict using both models with priority logic:
        - Model 1: Numbers (0-9) ONLY
        - Model 2: All ASL letters (A-Z) + special commands (del, nothing, space)
        ...
        if self.model2 is not None:
            with torch.no_grad():
                ...
                predictions.append({
                    ...
                    'is_letter': label2.isalpha() and label2 not in ['del', 'nothing', 'space']
                })
        ...
        if len(predictions) == 2:
            ...
            # PRIORITY 1: If Model 1 predicts a NUMBER (0-9) → Use Model 1
            # Model 2 doesn't have numbers, so Model 1 is the only option
            if pred1['is_number'] and pred1['confidence'] > 0.4:
                return pred1['label'], pred1['confidence'], pred1['model']
            
            # PRIORITY 2: If Model 2 predicts SPECIAL COMMANDS → Use Model 2
            # (del, nothing, space) - Model 1 doesn't have these
            if pred2['is_special'] and pred2['confidence'] > 0.5:
                return pred2['label'], pred2['confidence'], pred2['model']
            
            # PRIORITY 3: For ASL LETTERS → ALWAYS use Model 2 (more accurate)
            # Model 2 is trained specifically on ASL dataset, so it's more reliable
            if pred2['is_letter'] and pred2['confidence'] > 0.4:
                return pred2['label'], pred2['confidence'], pred2['model']
            
            # FALLBACK: If Model 2 confidence is too low, check Model 1
            # Only use Model 1 for letters if Model 2 has very low confidence
            if pred1['is_letter'] and pred1['confidence'] > 0.5 and pred2['confidence'] < 0.3:
                return pred1['label'].upper(), pred1['confidence'], pred1['model']  # Convert to uppercase
            
            # DEFAULT: Use Model 2 (it's the primary ASL model)
            return pred2['label'], pred2['confidence'], pred2['model']
...
while True:
    ...
    # Show active models and priority
    models_text = "Priority: Model 2 (ASL A-Z, del, space) | Model 1 (Numbers 0-9)"
